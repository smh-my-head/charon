...
cur_filename="MERGE_CURRENT.sldprt"
mrg_filename="MERGE_OTHER.sldprt"
cp $2 $cur_filename
cp $3 $mrg_filename

while true; do
	read -p "Conflicting part found. Do you want to merge manually? (y/n) " yn
	case "$yn" in
		"y" )
			echo "Parts to merge are $cur_filename and $mrg_filename."
